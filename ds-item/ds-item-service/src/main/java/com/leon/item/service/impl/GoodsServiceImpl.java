...
import com.leon.item.bo.SecondKillParameter;
import com.leon.item.mapper.*;
import com.leon.item.pojo.*;
import com.leon.item.service.CategoryService;
import com.leon.item.service.GoodsService;
...
public class GoodsServiceImpl implements GoodsService {
    ...
    public void saveGoods(SpuBo spu) {
        ...
        System.out.println(spuDetail.getGenericSpec().length());
    ...
    */
    ...
    public SpuBo queryGoodsById(Long id) {
        ...
        SpuBo spuBo = new SpuBo(spu.getBrandId(), spu.getCid1(), spu.getCid2(), spu.getCid3(),
    ...
    */
    @Override
    public List<SecondKillGoods> querySecondKillGoods() {
        Example example = new Example(SecondKillParameter.class);
        ...
        list.forEach(goods -> {
            ...
            goods.setSecondKillTotal(stock.getSeckillTotal());
        });
        return list;
    }
    ...
    */
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void addSecondKillGoods(SecondKillParameter secondKillParameter) throws ParseException {
        SimpleDateFormat sdf =  new SimpleDateFormat( "yyyy-MM-dd HH:mm" );
        //1.根据spu_id查询商品
        Long id = secondKillParameter.getId();
        Sku sku = this.querySkuById(id);
        //2.插入到秒杀商品表中
        SecondKillGoods secondKillGoods = new SecondKillGoods();
        secondKillGoods.setEnable(true);
        secondKillGoods.setStartTime(sdf.parse(secondKillParameter.getStartTime().trim()));
        secondKillGoods.setEndTime(sdf.parse(secondKillParameter.getEndTime().trim()));
        secondKillGoods.setImage(sku.getImages());
        secondKillGoods.setSkuId(sku.getId());
        secondKillGoods.setStock(secondKillParameter.getCount());
        secondKillGoods.setTitle(sku.getTitle());
        secondKillGoods.setSecondKillPrice(sku.getPrice()*secondKillParameter.getDiscount());
        this.secondKillMapper.insert(secondKillGoods);
        //3.更新对应的库存信息，tb_stock
        Stock stock = stockMapper.selectByPrimaryKey(sku.getId());
        System.out.println(stock);
        if (stock != null) {
            stock.setSeckillStock(stock.getSeckillStock() != null ? stock.getSeckillStock() + secondKillParameter.getCount() : secondKillParameter.getCount());
            stock.setSeckillTotal(stock.getSeckillTotal() != null ? stock.getSeckillTotal() + secondKillParameter.getCount() : secondKillParameter.getCount());
            stock.setStock(stock.getStock() - secondKillParameter.getCount());
        ...
        //4.更新redis中的秒杀库存
        updateSecondKillStock();
    }

    /**
    * @description: 增加sku和库存信息
    * @param skus
	* @param id
    * @return void
    */
    ...
    */
    ...
    /**
    * @description: 更新秒杀商品数量
    * @param
    * @return void
    */
    public void updateSecondKillStock(){
        //查询可以秒杀的商品
        List<SecondKillGoods> seckillGoods = this.querySecondKillGoods();
